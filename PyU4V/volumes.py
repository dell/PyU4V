...
class VolumesFunctions(object):
    ...
    def get_volumes_meta_data(self):
        """Get details on available meta data.

        :returns: dictionary with list of attributes and descriptions of
                 attribute types -- dict
        """
        response = constants.VOLUMES_METADATA
        return response

    def get_volumes_details(self, array_id=None, filters=None, select=None,
                            exclude=None):
        ...
        if not select:
            select = []
            volume_metadata = self.get_volumes_meta_data()
        ...
        response = self.common.get_request(
            target_uri=f"/{self.enhanced_api_version}/systems"
                       f"/{array_id}/volumes?select={select}{filters}",
                       resource_type=None)
        return response
